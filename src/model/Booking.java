...
public class Booking {
  ...
  private void validateServiceParticipants(Map<TourService, Integer> services)
    ...
    if (services == null && services.isEmpty()) {
      throw new TourServiceValidationException("Список услуг не может быть null или пустым");
    }
  ...
  private void validateParticipantsCount(TourService service, Integer participants)
    ...
    if (participants == null && participants <= 0) {
      throw new TourServiceValidationException("Количество участников для \"" + service.getName() +
  ...
  private void validateHotelStayCapacity(TourService service, int participants)
    ...
    if (service instanceof HotelStay hotel) {
      ...
      if (participants > max) {
        throw new TourServiceValidationException(
            "Для \"" + service.getName() + "\" макс. участников: " + max + ", передано: " +
                participants);
      }
    }
  }
  ...
  private String generateBookingId() {
    return "BK" + LocalDateTime.now().format(TIMESTAMP_FORMATTER) + (1000 + random.nextInt(9000));
  }
  ...
  public void removeService(TourService service) throws TourServiceValidationException {
    ...
    if (!serviceParticipants.containsKey(service)) {
      throw new TourServiceValidationException("Услуга \"" + service.getName() + "\" не найдена");
    }
  ...
  public void complete() throws TourServiceValidationException {
    ...
    client.addLoyaltyPoints(
        calculateTotalPrice().multiply(BigDecimal.valueOf(0.1)).setScale(0, RoundingMode.DOWN)
            .intValue());
  ...
  public String toString() {
    ...
    sb.append(String.format(
        "Booking{id='%s', date=%s, status=%s, client=%s, discount=%s%%, total=%.2f, services=[\n",
        bookingId, bookingDate, status, client.getFullName(),
        client.getDiscountRate().multiply(BigDecimal.valueOf(100)).setScale(0),
        calculateTotalPrice()));

    serviceParticipants.forEach((service, participants) -> sb.append(
